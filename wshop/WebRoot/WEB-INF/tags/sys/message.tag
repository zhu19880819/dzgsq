...
<c:if test="${not empty content}">
	...
	<div id="messageBox" class="alert alert-${ctype} ${empty content ? 'hide':''}">
		<button data-dismiss="alert" class="close" type="button" onclick="hide()">×</button>${content}
	</div> 
	<script type="text/javascript">
	if(!$.jBox.tip.mess){
		$.jBox.tip.mess=1;
		$.jBox.tip("${content}","${ctype}",{persistent:true,opacity:0});
		$("#messageBox").show();
	}
	function hide(){
		$("#messageBox").hide();
