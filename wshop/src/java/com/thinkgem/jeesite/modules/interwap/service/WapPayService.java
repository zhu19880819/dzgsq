package com.thinkgem.jeesite.modules.interwap.service;
...
public class WapPayService extends BaseService{
	...
	public Map<String,Object> orderPay(WsMember member,List<WsOrderItem> wsOrderItems) throws Exception {
	...
	@Transactional(readOnly = false)
	public Map orderPayPre(String orderId,WsMember member,String ip,String notify,String buysWords,String ruid) throws Exception {
		Map data=new HashMap();
		/**
		 * 查询订单信息
		 */
        ...
        data.put("ret",InterConstant.RET_SUCCESS);
        data.put("wxurl",r.get("mweb_url"));
        wsOrder.setOrderSn(orderSn);
		if(StringUtils.isNotEmpty(buysWords) && !buysWords.equals("null")&& !buysWords.equals("undefined")) {
			wsOrder.setBuysWords(buysWords);
		}
		if(StringUtils.isNotEmpty(ruid)&& !ruid.equals("null")&& !ruid.equals("undefined")){
			wsOrder.setRuid(new WsMember(ruid));
		}
		wsOrderService.save(wsOrder);
		return data;
	}
