...
import com.thinkgem.jeesite.modules.sys.task.QuartzJobFactory;
import com.thinkgem.jeesite.modules.sys.task.QuartzJobFactoryDisallowConcurrentExecution;
...
public class SysScheduleJobService extends CrudService<SysScheduleJobDao, SysScheduleJob> {
	...
	public void addJob(SysScheduleJob job) throws SchedulerException {
		...
		if (null == trigger) {
			...
			JobDetail jobDetail = JobBuilder.newJob(clazz).withIdentity(job.getJobName(), job.getJobGroup()).usingJobData(jobDataMap).requestRecovery(true).storeDurably(true).build();
